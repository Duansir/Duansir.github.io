...
![](F:\Duansir.github.io\images\posts\互补滤波原理.jpg)

低通滤波器：
$$
F_{lp}(s) = \cfrac{C(s)}{C(s)+s}
$$
高通滤波器：
$$
F_{hp}(s) = 1 -F_{lp}(s)= \cfrac{s}{C(s)+s}
$$
...
![](F:\Duansir.github.io\images\posts\控制图.jpg)
...
![](F:\Duansir.github.io\images\posts\高低通滤波器.jpg)



随着 C(s)取不同的形式，两个滤波器的阶次也会随之变化，比如：
$$
当C(s)= \alpha时，滤波器阶数为一阶。F_{lp}(s) = \cfrac{\alpha}{\alpha+s}, 即时间常数为\cfrac{1}{\alpha}的一阶低通滤波器，\\F_{hp}(s) = 1 -F_{lp}(s)= \cfrac{s}{\alpha+s}
$$

$$
当C(s)= \alpha + \cfrac{\beta}{s}时，滤波器阶数为二阶。F_{lp}(s) = \cfrac{\alpha + \cfrac{\beta}{s}}{\alpha + \cfrac{\beta}{s}+s}=\cfrac{\alpha s+\beta}{s^2+\alpha s+\beta}, \\F_{hp}(s) = 1 -F_{lp}(s)= \cfrac{s^2}{s^2+\alpha s+\beta}
$$
...
![](F:\Duansir.github.io\images\posts\互补滤波.jpg)

实际中采用的姿态互补滤波器（PI控制）

![](F:\Duansir.github.io\images\posts\PI.jpg)
...
![](F:\Duansir.github.io\images\posts\complementary-filter-01.png)
...
![](F:\Duansir.github.io\images\posts\Mahony.png)
$$
从图中可以看出，加速度传感器测的数据f^b并没有直接被g^b拿来使用。而是通过\bar\Omega^b×V_{air}^b进行了修正。\\它的目的就是消除飞行过程中的运行加速度，从而更好的还原重力加速度。
$$
...
用*k-1*时刻的值来预测当前*k*时刻的状态值***X(K|K-1)***和误差协方差***P (K|K-1)***， ***(K|K-1)***表示用*K-1*时刻的值来算*K*时刻，表示是预测的。

***(K-1|K-1)***表示*K-1*时刻的最优值。***(K|K)***表示*K*时刻的最优值，是最终需要得到的值。
